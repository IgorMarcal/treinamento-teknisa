function validaCPF(){
    ...
    if(verificaDigitosRepetidos(cpf)){
        mostraResultado('CPF não pode conter repetição do mesmo numero', 'red');
    }

    const digito1 = calcularDigitoVerificador(cpf, 1);
    if(!digito1 ){
        return mostraResultado("CPF inválido - " + cpf, 'red');
    }
    ...
    return mostraResultado("CPF válido - " + cpf , 'green');
